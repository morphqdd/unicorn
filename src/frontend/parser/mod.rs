...
parser! {
    pub grammar parser() for str {
        ...
        rule exprs() -> Vec<Expr> = e:(expr() ** _) { e }
        rule expr() -> Expr = function() / assign() /ident() / literal()
        rule assign() -> Expr = _ "let" _ i:ident() _ ":" _ t:ty() _ "=" _ e:expr() { Expr::Assign((Box::new(i), Box::new(t)), Box::new(e)) }  
...
mod test {
        ...
        assert_eq!(
            ...
            Ok(Expr::Function {
                ...
                function_ty: Box::new(
                    Expr::FunctionType { 
                        params: vec![
                            (Expr::Ident(String::from("a")), Expr::Ident(String::from("T"))),
                            (Expr::Ident(String::from("b")), Expr::Ident(String::from("None")))
                        ],
    ...
    fn complicated_function_parse() {
        assert_eq!(
            parser::function("foo : bar( a(i32) b(i32) -> nil ) -> nil { buzz : a(i32) b(i32) -> nil {} nil }"),
            Ok(Expr::Function {
    ...
    fn let_expr_parse() {
        assert_eq!(
            ...
            Ok(Expr::Function { 
                ...
                body: vec![
                    Expr::Assign((Box::new(Expr::Ident("a".into())), Box::new(Expr::Ident("i32".into()))), Box::new(Expr::Lit("20".into()))),
                    Expr::Ident("nil".into())
                ]
            })
        )
